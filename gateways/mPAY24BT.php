...
{	
    ...
    $request = new mPAY24BT_SelectPayment($mode);
	...
	$request->addPaymentType("EPS");			// all available EPS Brands
	$request->addPaymentType("GIROPAY");		// giropay

	// add users address-data to this order
   	$billing_addr = new mPAY24BT_Address();
	...
	$confirmUrl = $params['systemurl'] . '/modules/gateways/callback/mPAY24BT.php?' . implode('&', $urlParams);
...
class mPAY24BT_SelectPayment {

	// constructor
	function mPAY24BT_SelectPayment($url) {
		$etp_url = "https://";
	   if($url != "www")
		   $etp_url .= "test";
		else
