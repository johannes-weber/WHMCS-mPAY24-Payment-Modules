<?php
/**
 * WHMCS Paymentmodule for mPay24 Creditcard Payment Gateway
 * 
 ...
 * @return array
 */
function mPAY24CC_config()
{
	$configarray = array(
		"FriendlyName" => array("Type" => "System", "Value" => "mPAY24 Credit Card"),
		"partnerID" => array("FriendlyName" => "mPAY24 Partner ID", "Type" => "text", "Size" => "10",),
		...
		"logoUrl" => array("FriendlyName" => "Logo Url", "Type" => "text", "Description" => "This image were displayed in customers digital bill."),
		"linkText" => array("FriendlyName" => "Link-Text", "Type" => "text", "Description" => "Link name which is placed under the payment image"),
		"wbmiTitle" => array("FriendlyName" => "WBMI Title", "Type" => "text", "Description" => "This text will be displayed on mPAY24 external payment window."),
		...
		"notificationSubject" => array("FriendlyName" => "Notification E-Mail subject", "Type" => "text", "Description" => "Default is 'WHMCS order notification'"),
		"notificationSenderName" => array("FriendlyName" => "Notification E-Mail sender Name", "Type" => "text", "Description" => "Default is 'WHMCS order notification'"),
		"notificationSenderEmail" => array("FriendlyName" => "Notification sender E-Mail", "Type" => "text", "Description" => "Default is 'noreply@noreply.com'"),
		"testmode" => array("FriendlyName" => "Test Mode", "Type" => "yesno", "Description" => "Is in test mode",),
	);
 ...
 * ************************************************************************* */
class mPAY24CC_SelectPayment
{

	// constructor
	function mPAY24CC_SelectPayment($url)
	{
		$etp_url = "https://";
		if($url != "www") $etp_url .= "test";
		else $etp_url .= "www";
	...
	function createXmlFile()
	{
		...
		if(phpversion() >= 5)
		{
			...
			if($this->style->getLogoStyle()) $xmlOrder->setAttribute('LogoStyle', $this->style->getLogoStyle());
			if($this->style->getPageHeaderStyle()) $xmlOrder->setAttribute('PageHeaderStyle', $this->style->getPageHeaderStyle());
			if($this->style->getPageCaptionStyle()) $xmlOrder->setAttribute('PageCaptionStyle', $this->style->getPageCaptionStyle());
			if($this->style->getPageStyle()) $xmlOrder->setAttribute('PageStyle', $this->style->getPageStyle());
			if($this->style->getFooterStyle()) $xmlOrder->setAttribute('FooterStyle', $this->style->getFooterStyle());
			if($this->style->getWholePageStyle()) $xmlOrder->setAttribute("Style", $this->style->getWholePageStyle());
			if($this->style->getInputFieldsStyle()) $xmlOrder->setAttribute("InputFieldsStyle", $this->style->getInputFieldsStyle());
			...
			if(!empty($this->paymenttypes))
			{
				...
				if($this->ptenable) $xmlPaymentTypes->setAttribute('Enable', $this->ptenable);
			...
			if($this->style->getSCStyle()) $xmlShoppingCart->setAttribute('Style', $this->style->getSCStyle());
			if($this->style->getSCHeader()) $xmlShoppingCart->setAttribute('Header', $this->style->getSCHeader());
			if($this->style->getSCHeaderStyle()) $xmlShoppingCart->setAttribute('HeaderStyle', $this->style->getSCHeaderStyle());
			if($this->style->getSCCaptionStyle()) $xmlShoppingCart->setAttribute('CaptionStyle', $this->style->getSCCaptionStyle());
			if($this->style->getSCNumberStyle()) $xmlShoppingCart->setAttribute('NumberStyle', $this->style->getSCNumberStyle());
			if($this->style->getSCProductNrStyle()) $xmlShoppingCart->setAttribute('ProductNrStyle', $this->style->getSCProductNrStyle());
			if($this->style->getSCDescriptionStyle()) $xmlShoppingCart->setAttribute('DescriptionStyle', $this->style->getSCDescriptionStyle());
			if($this->style->getSCPackageStyle()) $xmlShoppingCart->setAttribute('PackageStyle', $this->style->getSCPackageStyle());
			if($this->style->getSCQuantityStyle()) $xmlShoppingCart->setAttribute('QuantityStyle', $this->style->getSCQuantityStyle());
			if($this->style->getSCItemPriceStyle()) $xmlShoppingCart->setAttribute('ItemPriceStyle', $this->style->getSCItemPriceStyle());
			if($this->style->getSCPriceStyle()) $xmlShoppingCart->setAttribute('PriceStyle', $this->style->getSCPriceStyle());
			...
			for($i = 0; $i < count($this->itemlist); $i++)
			{
				...
				{
					if($xmlNumber) $xmlNumber->setAttribute('Style', $this->style->getSCItemEvenStyle());
					if($xmlProductNumber) $xmlProductNumber->setAttribute('Style', $this->style->getSCItemEvenStyle());
					if($xmlDescription) $xmlDescription->setAttribute('Style', $this->style->getSCItemEvenStyle());
					if($xmlPackage) $xmlPackage->setAttribute('Style', $this->style->getSCItemEvenStyle());
					if($xmlQuantity) $xmlQuantity->setAttribute('Style', $this->style->getSCItemEvenStyle());
					if($xmlItemPrice) $xmlItemPrice->setAttribute('Style', $this->style->getSCItemEvenStyle());
					if($xmlPrice) $xmlPrice->setAttribute('Style', $this->style->getSCItemEvenStyle());
				} elseif(!($i % 2) && $this->style->getSCItemOddStyle())
				{
					if($xmlNumber) $xmlNumber->setAttribute('Style', $this->style->getSCItemOddStyle());
					if($xmlProductNumber) $xmlProductNumber->setAttribute('Style', $this->style->getSCItemOddStyle());
					if($xmlDescription) $xmlDescription->setAttribute('Style', $this->style->getSCItemOddStyle());
					if($xmlPackage) $xmlPackage->setAttribute('Style', $this->style->getSCItemOddStyle());
					if($xmlQuantity) $xmlQuantity->setAttribute('Style', $this->style->getSCItemOddStyle());
					if($xmlItemPrice) $xmlItemPrice->setAttribute('Style', $this->style->getSCItemOddStyle());
					if($xmlPrice) $xmlPrice->setAttribute('Style', $this->style->getSCItemOddStyle());
				}
			}
			...
			for($i = 0; $i < count($this->shippingcosts); $i++)
			{
				...
				if($this->style->getShippingCostsHStyle()) $xmlShippingCosts->setAttribute('HeaderStyle', $this->style->getShippingCostsHStyle());
				if($this->style->getShippingCostsStyle()) $xmlShippingCosts->setAttribute('Style', $this->style->getShippingCostsStyle());
			}
			...
			for($i = 0; $i < count($this->tax); $i++)
			{
				...
				if($this->style->getTaxHStyle()) $xmlTax->setAttribute('HeaderStyle', $this->style->getTaxHStyle());
				if($this->style->getTaxStyle()) $xmlTax->setAttribute('Style', $this->style->getTaxStyle());
			}
			$xmlPrice = $doc->createElement('Price', $this->total_price);
			$xmlPrice = $xmlOrder->appendChild($xmlPrice);
			if($this->style->getPRHeader()) $xmlPrice->setAttribute('Header', $this->style->getPRHeader());
			if($this->style->getPRHeaderStyle()) $xmlPrice->setAttribute('HeaderStyle', $this->style->getPRHeaderStyle());
			if($this->style->getPRStyle()) $xmlPrice->setAttribute('Style', $this->style->getPRStyle());
		...
		else
		{
			$this->xmlfile = "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n" .
				"<Order";
			if($this->style->getLogoStyle()) $this->xmlfile .= " LogoStyle=\"" . $this->style->getLogoStyle() . "\"";
			if($this->style->getPageHeaderStyle()) $this->xmlfile .= " PageHeaderStyle=\"" . $this->style->getPageHeaderStyle() . "\"";
			if($this->style->getPageCaptionStyle()) $this->xmlfile .= " PageCaptionStyle=\"" . $this->style->getPageCaptionStyle() . "\"";
			if($this->style->getPageStyle()) $this->xmlfile .= " PageStyle=\"" . $this->style->getPageStyle() . "\"";
			if($this->style->getFooterStyle()) $this->xmlfile .= " FooterStyle=\"" . $this->style->getFooterStyle() . "\"";
			if($this->style->getWholePageStyle()) $this->xmlfile .= " Style=\"" . $this->style->getWholePageStyle() . "\"";
			if($this->style->getInputFieldsStyle()) $this->xmlfile .= " InputFieldsStyle=\"" . $this->style->getInputFieldsStyle() . "\"";
			$this->xmlfile .= ">\n";

			if(!empty($this->userfield)) $this->xmlfile .= " <UserField>" . $this->userfield . "</UserField>\n";
			$this->xmlfile .= " <Tid>" . $this->tid . "</Tid>\n";

			if(!empty($this->TemplateSet) || !empty($this->language))
			{
				$this->xmlfile .= " <TemplateSet";
				if(!empty($this->language)) $this->xmlfile .= " Language=\"" . $this->language . "\"";
			...
			if(!empty($this->paymenttypes))
			{
				...
				if($this->ptenable) $this->xmlfile .= " Enable=\"" . $this->ptenable . "\"";
				$this->xmlfile .= ">\n";
				for($i = 0; $i < count($this->paymenttypes); $i++)
				{
				...
				!empty($this->itemlist))
			{
				...
				if($this->style->getSCStyle()) $this->xmlfile .= " Style=\"" . $this->style->getSCStyle() . "\"";
				if($this->style->getSCHeader()) $this->xmlfile .= " Header=\"" . $this->style->getSCHeader() . "\"";
				if($this->style->getSCHeaderStyle()) $this->xmlfile .= " HeaderStyle=\"" . $this->style->getSCHeaderStyle() . "\"";
				if($this->style->getSCCaptionStyle()) $this->xmlfile .= " CaptionStyle=\"" . $this->style->getSCCaptionStyle() . "\"";
				if($this->style->getSCNumberStyle()) $this->xmlfile .= " NumberStyle=\"" . $this->style->getSCNumberStyle() . "\"";
				if($this->style->getSCProductNrStyle()) $this->xmlfile .= " ProductNrStyle=\"" . $this->style->getSCProductNrStyle() . "\"";
				if($this->style->getSCDescriptionStyle()) $this->xmlfile .= " DescriptionStyle=\"" . $this->style->getSCDescriptionStyle() . "\"";
				if($this->style->getSCPackageStyle()) $this->xmlfile .= " PackageStyle=\"" . $this->style->getSCPackageStyle() . "\"";
				if($this->style->getSCQuantityStyle()) $this->xmlfile .= " QuantityStyle=\"" . $this->style->getSCQuantityStyle() . "\"";
				if($this->style->getSCItemPriceStyle()) $this->xmlfile .= " ItemPriceStyle=\"" . $this->style->getSCItemPriceStyle() . "\"";
				if($this->style->getSCPriceStyle()) $this->xmlfile .= " PriceStyle=\"" . $this->style->getSCPriceStyle() . "\"";
				$this->xmlfile .= ">\n";
				if(!empty($this->description)) $this->xmlfile .= "  <Description>" . $this->description . "</Description>\n";
				for($i = 0; $i < count($this->itemlist); $i++)
				{
					$this->xmlfile .= "  <Item>\n";
					if($this->itemlist[$i]->getNumber())
					{
						$this->xmlfile .= "<Number";
						if(($i % 2) && $this->style->getSCItemEvenStyle()) $this->xmlfile .= " Style=\"" . $this->style->getSCItemEvenStyle() . "\"";
						elseif(!($i % 2) && $this->style->getSCItemOddStyle()) $this->xmlfile .= " Style=\"" . $this->style->getSCItemOddStyle() . "\"";
				...
				for($i = 0; $i < count($this->shippingcosts); $i++)
				{
					...
					if($this->style->getShippingCostsHStyle()) $this->xmlfile .= " HeaderStyle=\"" . $this->style->getShippingCostsHStyle() . "\"";
					if($this->style->getShippingCostsStyle()) $this->xmlfile .= " Style=\"" . $this->style->getShippingCostsStyle() . "\"";
					if(strpos($this->shippingcosts[$i], ",") == false) $this->xmlfile .= ">" . number_format($this->shippingcosts[$i], 2, '.', '') . "</ShippingCosts>\n";
					else
					{
						list($header, $value) = split(",", $this->shippingcosts[$i]);
				...
				{
					$this->xmlfile .= "  <Tax";
					if($this->style->getTaxHStyle()) $this->xmlfile .= " HeaderStyle=\"" . $this->style->getTaxHStyle() . "\"";
					if($this->style->getTaxStyle()) $this->xmlfile .= " Style=\"" . $this->style->getTaxStyle() . "\"";
					if(strpos($this->tax[$i], ",") == false) $this->xmlfile .= ">" . number_format($this->tax[$i], 2, '.', '') . "</Tax>\n";
					else
					{
						list($value1, $value2, $value3) = split(",", $this->tax[$i]);
						if($value3) $this->xmlfile .= " Percent=\"" . $value1 . "\" Header=\"" . $value2 . "\">" . number_format($value3, 2, '.', '') . "</Tax>\n";
						elseif(is_numeric($value1) && $value3 == false) $this->xmlfile .= " Percent=\"" . $value1 . "\">" . number_format($value2, 2, '.', '') . "</Tax>\n";
						else $this->xmlfile .= " Header=\"" . $value1 . "\">" . number_format($value2, 2, '.', '') . "</Tax>\n";
					}
				}
				$this->xmlfile .= " </ShoppingCart>\n";
			}
			$this->xmlfile .= " <Price";
			if($this->style->getPRHeader()) $this->xmlfile .= " Header=\"" . $this->style->getPRHeader() . "\"";
			if($this->style->getPRHeaderStyle()) $this->xmlfile .= " HeaderStyle=\"" . $this->style->getPRHeaderStyle() . "\"";
			if($this->style->getPRStyle()) $this->xmlfile .= " Style=\"" . $this->style->getPRStyle() . "\"";
			$this->xmlfile .= ">" . number_format($this->total_price, 2, '.', '') . "</Price>\n";
			if($this->currency) $this->xmlfile .= " <Currency>" . $this->currency . "</Currency>\n";
			if($this->customer_details)
			{
				list($id, $set) = split(",", $this->customer_details);
			...
			if($this->billing_addr)
			{
				...
				if($this->billing_addr->getEmail()) $this->xmlfile .= "  <Email>" . $this->xmlentities($this->billing_addr->getEmail()) . "</Email>\n";
			...
			{
				$this->xmlfile .= " <URL>\n";
				if($this->success_url) $this->xmlfile .= "  <Success>" . $this->xmlentities($this->success_url) . "</Success>\n";
				if($this->error_url) $this->xmlfile .= "  <Error>" . $this->xmlentities($this->error_url) . "</Error>\n";
				if($this->confirm_url) $this->xmlfile .= "  <Confirmation>" . $this->xmlentities($this->confirm_url) . "</Confirmation>\n";
				$this->xmlfile .= " </URL>\n";
			}
	...
	function send()
	{
		if($this->isValid())
		{
			...
			if(class_exists("SoapClienttt"))
			{
				...
				if($this->etp_url == "https://test.mPAY24.com/app/bin/etpv5") $client->__setLocation('https://test.mPAY24.com/app/bin/etpproxy_v14');
